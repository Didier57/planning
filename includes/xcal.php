...
function save_uid_for_event ( $importId, $id, $uid ) {
  ...
  dbi_execute ( 'DELETE FROM webcal_import_data
  WHERE cal_id = ?
  ...
  // do_debug ( "SQL: $sql" );
  if ( ! dbi_execute ( $sql, [$importId, $id, $login, 'publish', $uid] ) ) {
    $error = db_error();
    // do_debug ( $error );
  }
...
function create_import_instance($username='') {
  ...
  if ( ! dbi_execute ( $sql, [$importId, $name, date ( 'Ymd' ), 'publish', $login] ) ) {
    $error = db_error();
    return;
  }
  return ( $importId );
...
function import_data ( $data, $overwrite, $type, $silent=false ) {
  ...
  foreach ( $data as $Entry ) {
    ...
    AND cal_external_id = ?', [$login, $Entry['UID']] );
      if ( $res ) {
        if ( $row = dbi_fetch_row ( $res ) ) {
          if ( ! empty ( $row[0] ) ) {
            $id = $row[0];
    ...
    if ( empty ( $error ) ) {
      ...
      if ( ! $updateMode ) {
        $sql = 'INSERT INTO webcal_entry_user
          ...
          VALUES ( ?, ?, ?, ? )';
        // do_debug ( "SQL> $sql" );
        if ( ! dbi_execute ( $sql, [$id, $participants[0], $status, $percent] ) ) {
          $error = db_error();
    ...
    if ( $overwrite && count ( $oldUIDs ) > 0 ) {
      ...
      for ( $i = 0; $i < $oldcnt; $i++ ) {
        ...
        $res = dbi_execute ( $sql, [$type, $old[$i], $calUser, $firstEventId] );
        if ( $res ) {
          while ( $row = dbi_fetch_row ( $res ) ) {
            $oldIds[] = $row[0];
          }
...
function RepeatType ( $type ) {
  $Repeat = [0, 'daily', 'weekly', 'monthlyByDay', 'monthlyByDate',
    ...
    if ( $res ) {
      if ( $row = dbi_fetch_row ( $res ) ) {
        ...
        if ( $res ) {
          ...
          if ( ! dbi_execute ( $sql, [$id, $login, $cat_name] ) ) {
            $error = db_error();
            // do_debug ( $error );
          } else {
            $ret[] = $id;
...
function get_vtimezone ( $tzid, $dtstart, $dtend='' ) {
  ...
  $res = dbi_execute ( $sql, [$tzid, $dtstart, $dtend] );
